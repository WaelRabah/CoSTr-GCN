...
def get_window_label(label):
    ...
    return  out if dominant_class_ratio > 0.8 else torch.tensor([0])
...
if __name__ == "__main__":
    ...
    # .........inital model
    print("\n loading model.............")
    model = model = CoSTrGCN.load_from_checkpoint(checkpoint_path="./models/CoSTrGCN-SHREC21_2022-09-06_01_20_48/best_model-128-8-v2.ckpt",adjacency_matrix=graph, optimizer_params=optimizer_params, labels=labels, d_model=128,n_heads=8,num_classes=num_classes, dropout=dropout_rate)
    ...
    with torch.no_grad():
        ...
        for i, batch in tqdm(enumerate(test_loader), leave=False):
            ...
            res=[]
            for t in tqdm(range(2*stride,T-window_size+1,stride), leave=False):
                ...
                if d_wi < eps and d_wi_m_1 > 0 and d_wi_p_1 < 0 :
                    ...
                    if prob[0][score_list_labels[0]] < .7:
                        continue 
                    res.append({
